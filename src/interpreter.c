...
Value *env_get(Env *env, const char *name) {
    ...
    for (Env *e = env; e; e = e->parent) {
        ...
        for (int i = 0; i < e->cap; i++) {
            ...
            if (strcmp(e->slots[curr].key, name) == 0) return e->slots[curr].val;
        }
    }
...
bool env_set(Env *env, const char *name, Value val, bool is_const) {
    ...
    for (int i = 0; i < env->cap; i++) {
        ...
        if (env->slots[curr].key && strcmp(env->slots[curr].key, name) == 0) {
            ...
            env->slots[curr].is_const = is_const;
            return true;
        }
        if (!env->slots[curr].key) break;
    }
...
static Value builtin_dict_fn(Value *args, int argc) {
    ...
    if (argc == 1 && VAL_TYPE(args[0]) == VAL_DICT) {
        for (int i = 0; i < AS_DICT(args[0]).len; i++)
            value_dict_set(d, AS_DICT(args[0]).entries[i].key, AS_DICT(args[0]).entries[i].val);
    } else if (argc % 2 == 0) {
...
static Value builtin_abs(Value *a, int n) {
    ...
    if (VAL_TYPE(a[0]) == VAL_INT)   return value_int(llabs(AS_INT(a[0])));
    if (VAL_TYPE(a[0]) == VAL_FLOAT) return value_float(fabs(AS_FLOAT(a[0])));
...
static Value builtin_floor(Value *a, int n) {
    if (n < 1) return value_int(0);
    if (VAL_TYPE(a[0]) == VAL_INT) return value_int(AS_INT(a[0]));
    return value_int((long long)floor(AS_FLOAT(a[0])));
}
static Value builtin_ceil(Value *a, int n) {
    if (n < 1) return value_int(0);
    if (VAL_TYPE(a[0]) == VAL_INT) return value_int(AS_INT(a[0]));
    return value_int((long long)ceil(AS_FLOAT(a[0])));
}
static Value builtin_round(Value *a, int n) {
    if (n < 1) return value_int(0);
    if (VAL_TYPE(a[0]) == VAL_INT) return value_int(AS_INT(a[0]));
    return value_int((long long)round(AS_FLOAT(a[0])));
}
static Value builtin_pow(Value *a, int n) {
    if (n < 2) return value_float(0.0);
    double base = (VAL_TYPE(a[0]) == VAL_INT) ? (double)AS_INT(a[0]) : AS_FLOAT(a[0]);
    double exp  = (VAL_TYPE(a[1]) == VAL_INT) ? (double)AS_INT(a[1]) : AS_FLOAT(a[1]);
    double r = pow(base, exp);
    if (VAL_TYPE(a[0]) == VAL_INT && VAL_TYPE(a[1]) == VAL_INT && exp >= 0)
...
static Value builtin_atan2(Value *a, int n) {
    ...
    double x=(VAL_TYPE(a[1])==VAL_INT)?(double)AS_INT(a[1]):AS_FLOAT(a[1]);
...
static Value builtin_hypot(Value *a, int n) {
    ...
    double y=(VAL_TYPE(a[1])==VAL_INT)?(double)AS_INT(a[1]):AS_FLOAT(a[1]);
...
static Value builtin_min(Value *a, int n) {
    ...
    if (n == 1 && VAL_TYPE(a[0]) == VAL_ARRAY) {
        if (AS_ARRAY(a[0]).len == 0) return value_null();
...
static Value builtin_upper(Value *a, int n) {
    if (n < 1 || VAL_TYPE(a[0]) != VAL_STRING) return n>0 ? value_retain(a[0]) : value_string("");
    char *r = strdup(AS_STR(a[0]));
...
static Value builtin_ends(Value *a, int n) {
    if (n < 2 || VAL_TYPE(a[0]) != VAL_STRING || VAL_TYPE(a[1]) != VAL_STRING) return value_bool(0);
    size_t sl = strlen(AS_STR(a[0])), pl = strlen(AS_STR(a[1]));
    return value_bool((sl >= pl && strcmp(AS_STR(a[0]) + sl - pl, AS_STR(a[1])) == 0) ? 1 : 0);
}
static Value builtin_contains(Value *a, int n) {
    if (n < 2) return value_bool(0);
    if (VAL_TYPE(a[0]) == VAL_STRING && VAL_TYPE(a[1]) == VAL_STRING)
        return value_bool(strstr(AS_STR(a[0]), AS_STR(a[1])) != NULL ? 1 : 0);
...
static Value builtin_split(Value *a, int n) {
    if (n < 1 || VAL_TYPE(a[0]) != VAL_STRING) return value_array_new();
    const char *s = AS_STR(a[0]);
    const char *delim = (n >= 2 && VAL_TYPE(a[1]) == VAL_STRING) ? AS_STR(a[1]) : " ";
    Value arr = value_array_new();
...
static Value builtin_join(Value *a, int n) {
    ...
    const char *sep = "";
    Value arr = VAL_SPEC_NULL;
    if (n >= 2) {
        if (VAL_TYPE(a[0]) == VAL_STRING && VAL_TYPE(a[1]) == VAL_ARRAY) { sep = AS_STR(a[0]); arr = a[1]; }
        else if (VAL_TYPE(a[0]) == VAL_ARRAY && VAL_TYPE(a[1]) == VAL_STRING) { arr = a[0]; sep = AS_STR(a[1]); }
        else if (VAL_TYPE(a[0]) == VAL_ARRAY) { arr = a[0]; }
    } else if (n == 1 && VAL_TYPE(a[0]) == VAL_ARRAY) {
        arr = a[0];
    }
    ...
    char *res = malloc(cap);
    res[0] = '\0';
    for (int i = 0; i < AS_ARRAY(arr).len; i++) {
        char *part = value_to_string(AS_ARRAY(arr).items[i]);
        size_t plen = strlen(part);
        while (sz + (int)plen + (int)dlen + 2 >= cap) { cap *= 2; res = realloc(res, cap); }
        memcpy(res + sz, part, plen); sz += plen; res[sz] = '\0';
        if (i < AS_ARRAY(arr).len - 1 && dlen > 0) { memcpy(res + sz, sep, dlen); sz += dlen; res[sz] = '\0'; }
        free(part);
    }
    return value_string_take(res);
}
static Value builtin_replace(Value *a, int n) {
    if (n < 3 || VAL_TYPE(a[0]) != VAL_STRING || VAL_TYPE(a[1]) != VAL_STRING || VAL_TYPE(a[2]) != VAL_STRING)
        return n > 0 ? value_retain(a[0]) : value_string("");
    const char *s = AS_STR(a[0]), *old = AS_STR(a[1]), *neww = AS_STR(a[2]);
...
static Value builtin_parseInt(Value *a, int n) {
    if (n < 1) return value_int(0);
    if (VAL_TYPE(a[0]) == VAL_INT) return value_retain(a[0]);
    if (VAL_TYPE(a[0]) == VAL_FLOAT) return value_int((long long)AS_FLOAT(a[0]));
    if (VAL_TYPE(a[0]) == VAL_STRING) {
        int base = (n >= 2 && VAL_TYPE(a[1]) == VAL_INT) ? (int)AS_INT(a[1]) : 10;
        return value_int(strtoll(AS_STR(a[0]), NULL, base));
    }
    return value_int(0);
}
static Value builtin_parseFloat(Value *a, int n) {
    if (n < 1) return value_float(0.0);
    if (VAL_TYPE(a[0]) == VAL_FLOAT) return value_retain(a[0]);
    if (VAL_TYPE(a[0]) == VAL_INT)   return value_float((double)AS_INT(a[0]));
    if (VAL_TYPE(a[0]) == VAL_STRING) return value_float(strtod(AS_STR(a[0]), NULL));
    return value_float(0.0);
}
...
static Value builtin_push(Value *a, int n) {
    if (n < 2 || VAL_TYPE(a[0]) != VAL_ARRAY) return value_null();
...
static Value builtin_pop(Value *a, int n) {
    ...
    long long idx = (n >= 2 && VAL_TYPE(a[1]) == VAL_INT) ? AS_INT(a[1]) : (long long)(AS_ARRAY(a[0]).len - 1);
    return value_array_pop(a[0], idx);
}
static Value builtin_sort(Value *a, int n) {
    if (n < 1 || VAL_TYPE(a[0]) != VAL_ARRAY) return n>0?value_retain(a[0]):value_array_new();
...
static Value builtin_slice(Value *a, int n) {
    if (n < 1) return value_array_new();
    long long start = (n >= 2 && VAL_TYPE(a[1]) == VAL_INT) ? AS_INT(a[1]) : 0;
    if (VAL_TYPE(a[0]) == VAL_STRING) {
        const char *s = AS_STR(a[0]);
...
static Value builtin_flatten(Value *a, int n) {
    ...
    for (int i = 0; i < AS_ARRAY(a[0]).len; i++) {
        ...
        if (VAL_TYPE(item) == VAL_ARRAY) {
            for (int j = 0; j < AS_ARRAY(item).len; j++) value_array_push(result, AS_ARRAY(item).items[j]);
        } else {
...
static Value builtin_range(Value *a, int n) {
    ...
    if (n == 1) {
        stop = (VAL_TYPE(a[0]) == VAL_INT) ? AS_INT(a[0]) : (long long)AS_FLOAT(a[0]);
    } else if (n >= 2) {
        start = (VAL_TYPE(a[0]) == VAL_INT) ? AS_INT(a[0]) : (long long)AS_FLOAT(a[0]);
        stop  = (VAL_TYPE(a[1]) == VAL_INT) ? AS_INT(a[1]) : (long long)AS_FLOAT(a[1]);
        if (n >= 3)
            step = (VAL_TYPE(a[2]) == VAL_INT) ? AS_INT(a[2]) : (long long)AS_FLOAT(a[2]);
    }
    if (step == 0) step = 1;
    Value arr = value_array_new();
    if (step > 0) {
...
static Value builtin_keys(Value *a, int n) {
    if (n < 1 || VAL_TYPE(a[0]) != VAL_DICT) return value_array_new();
    Value arr = value_array_new();
    for (int i = 0; i < AS_DICT(a[0]).len; i++) {
        Value k = AS_DICT(a[0]).entries[i].key;
        /* skip internal keys */
        if (VAL_TYPE(k) == VAL_STRING && AS_STR(k)[0] == '_' && AS_STR(k)[1] == '_') continue;
...
static Value builtin_values(Value *a, int n) {
    ...
    for (int i = 0; i < AS_DICT(a[0]).len; i++) {
        Value k = AS_DICT(a[0]).entries[i].key;
        if (VAL_TYPE(k) == VAL_STRING && AS_STR(k)[0] == '_' && AS_STR(k)[1] == '_') continue;
        value_array_push(arr, AS_DICT(a[0]).entries[i].val);
    }
    return arr;
}
static Value builtin_items(Value *a, int n) {
    ...
    for (int i = 0; i < AS_DICT(a[0]).len; i++) {
        ...
        value_array_push(pair, AS_DICT(a[0]).entries[i].val);
...
static Value builtin_exit(Value *a, int n) {
    int code = (n > 0 && VAL_TYPE(a[0]) == VAL_INT) ? (int)AS_INT(a[0]) : 0;
    exit(code);
    return value_null();
}
static Value builtin_isnan(Value *a, int n) {
    if (n < 1) return value_bool(0);
    if (VAL_TYPE(a[0]) == VAL_FLOAT) return value_bool(isnan(AS_FLOAT(a[0])) ? 1 : 0);
    return value_bool(0);
}
static Value builtin_isinf(Value *a, int n) {
    if (n < 1) return value_bool(0);
    if (VAL_TYPE(a[0]) == VAL_FLOAT) return value_bool(isinf(AS_FLOAT(a[0])) ? 1 : 0);
    return value_bool(0);
}
static Value builtin_sum(Value *a, int n) {
    ...
    if (VAL_TYPE(a[0]) == VAL_ARRAY) {
        ...
        for (int i = 0; i < AS_ARRAY(a[0]).len; i++) {
            Value v = AS_ARRAY(a[0]).items[i];
            if (VAL_TYPE(v) == VAL_FLOAT) { use_float = true; fsum += AS_FLOAT(v); }
            else if (VAL_TYPE(v) == VAL_INT) { isum += AS_INT(v); fsum += (double)AS_INT(v); }
        }
...
static Value builtin_zip(Value *a, int n) {
    ...
    for (int i = 0; i < minlen; i++) {
        ...
        for (int j = 0; j < n; j++) value_array_push(tuple, AS_ARRAY(a[j]).items[i]);
...
static Value builtin_enumerate(Value *a, int n) {
    ...
    for (int i = 0; i < AS_ARRAY(a[0]).len; i++) {
        ...
        value_array_push(pair, AS_ARRAY(a[0]).items[i]);
...
static Value builtin_hex(Value *a, int n) {
    if (n < 1 || VAL_TYPE(a[0]) != VAL_INT) return value_string("0x0");
    char buf[32];
    snprintf(buf, sizeof(buf), "0x%llx", (unsigned long long)AS_INT(a[0]));
    return value_string(buf);
}
static Value builtin_bin(Value *a, int n) {
    if (n < 1 || VAL_TYPE(a[0]) != VAL_INT) return value_string("0b0");
    long long v = AS_INT(a[0]);
...
static Value builtin_oct(Value *a, int n) {
    if (n < 1 || VAL_TYPE(a[0]) != VAL_INT) return value_string("0o0");
    char buf[32];
    snprintf(buf, sizeof(buf), "0o%llo", (unsigned long long)AS_INT(a[0]));
    return value_string(buf);
}
...
static Value builtin_gui_window(Value *args, int argc) {
    if (argc >= 1 && VAL_TYPE(args[0]) == VAL_STRING)
        snprintf(g_gui.title, sizeof(g_gui.title), "%s", AS_STR(args[0]));
    if (argc >= 2 && VAL_TYPE(args[1]) == VAL_INT) g_gui.width  = (int)AS_INT(args[1]);
    if (argc >= 3 && VAL_TYPE(args[2]) == VAL_INT) g_gui.height = (int)AS_INT(args[2]);
    g_gui.active = 1;
    return value_null();
}
...
static Value builtin_gui_label(Value *args, int argc) {
    char buf[2048];
    const char *text = (argc >= 1 && VAL_TYPE(args[0]) == VAL_STRING) ? AS_STR(args[0]) : "";
...
static Value builtin_gui_button(Value *args, int argc) {
    char buf[2048];
    const char *text = (argc >= 1 && VAL_TYPE(args[0]) == VAL_STRING) ? AS_STR(args[0]) : "Button";
...
static Value bi_xgui_style(Value *args, int argc) {
    if (g_xgui && argc > 0 && VAL_TYPE(args[0]) == VAL_STRING)
        xgui_load_style(g_xgui, AS_STR(args[0]));
    return value_null();
}
...
static Value bi_xgui_label(Value *args, int argc) {
    if (g_xgui && argc > 0 && VAL_TYPE(args[0]) == VAL_STRING)
        xgui_label(g_xgui, AS_STR(args[0]));
    return value_null();
}

static Value bi_xgui_button(Value *args, int argc) {
    if (!g_xgui || argc < 1 || VAL_TYPE(args[0]) != VAL_STRING)
        return value_bool(0);
    return value_bool(xgui_button(g_xgui, AS_STR(args[0])) ? 1 : 0);
}
...
static Value bi_xgui_input(Value *args, int argc) {
    ...
    const char *placeholder = (argc > 1 && VAL_TYPE(args[1]) == VAL_STRING) ? AS_STR(args[1]) : "";
...
static Value bi_xgui_title(Value *args, int argc) {
    if (g_xgui && argc > 0 && VAL_TYPE(args[0]) == VAL_STRING)
        xgui_title(g_xgui, AS_STR(args[0]));
    return value_null();
}
static Value bi_xgui_subtitle(Value *args, int argc) {
    if (g_xgui && argc > 0 && VAL_TYPE(args[0]) == VAL_STRING)
        xgui_subtitle(g_xgui, AS_STR(args[0]));
    return value_null();
}
...
static Value bi_xgui_card_begin(Value *args, int argc) {
    (void)args; (void)argc;
    if (g_xgui) xgui_card_begin(g_xgui);
    return value_null();
}
static Value bi_xgui_card_end(Value *args, int argc) {
    (void)args; (void)argc;
    if (g_xgui) xgui_card_end(g_xgui);
    return value_null();
}
static Value bi_xgui_tooltip(Value *args, int argc) {
    if (!g_xgui || argc < 1) return value_null();
    const char *text = (VAL_TYPE(args[0]) == VAL_STRING) ? AS_STR(args[0]) : "";
    xgui_tooltip(g_xgui, text);
    return value_null();
}
...
static Value bi_xgui_tabs(Value *args, int argc) {
    ...
    int         count = (VAL_TYPE(args[2]) == VAL_INT)    ? (int)AS_INT(args[2]) : 0;
...
static Value bi_xgui_select(Value *args, int argc) {
    ...
    int         cur = (argc >= 3 && VAL_TYPE(args[2]) == VAL_INT) ? (int)AS_INT(args[2]) : 0;
...
static Value bi_xgui_section(Value *args, int argc) {
    if (!g_xgui || argc < 1) return value_null();
    xgui_section(g_xgui, (VAL_TYPE(args[0]) == VAL_STRING) ? AS_STR(args[0]) : "");
    return value_null();
}
...
static Value bi_xgui_clear_bg(Value *args, int argc) {
    if (!g_xgui || argc < 1) return value_null();
    uint32_t c = (VAL_TYPE(args[0]) == VAL_INT) ? (uint32_t)AS_INT(args[0]) : 0x000000;
    xgui_clear_bg(g_xgui, c);
    return value_null();
}
static Value bi_xgui_fill_rect_at(Value *args, int argc) {
    if (!g_xgui || argc < 5) return value_null();
    int x = (int)AS_INT(args[0]), y = (int)AS_INT(args[1]);
    int w = (int)AS_INT(args[2]), h = (int)AS_INT(args[3]);
...
static Value bi_xgui_fill_circle_at(Value *args, int argc) {
    ...
    xgui_fill_circle_at(g_xgui, (int)AS_INT(args[0]), (int)AS_INT(args[1]),
...
static Value bi_xgui_draw_line_at(Value *args, int argc) {
    ...
    uint32_t c = (argc >= 6) ? (uint32_t)AS_INT(args[5]) : (uint32_t)AS_INT(args[4]);
    xgui_draw_line_at(g_xgui, (int)AS_INT(args[0]), (int)AS_INT(args[1]),
...
static Value bi_xgui_draw_text_at(Value *args, int argc) {
    ...
    xgui_draw_text_at(g_xgui, (int)AS_INT(args[0]), (int)AS_INT(args[1]),
...
static Value bi_xgui_draw_text_centered(Value *args, int argc) {
    if (!g_xgui || argc < 5) return value_null();
    const char *t = (VAL_TYPE(args[2]) == VAL_STRING) ? AS_STR(args[2]) : "";
    xgui_draw_text_centered(g_xgui, (int)AS_INT(args[0]), (int)AS_INT(args[1]),
                            t, (int)AS_INT(args[3]), (uint32_t)AS_INT(args[4]));
    return value_null();
}
static Value bi_xgui_draw_text_bold_at(Value *args, int argc) {
    if (!g_xgui || argc < 5) return value_null();
    const char *t = (VAL_TYPE(args[2]) == VAL_STRING) ? AS_STR(args[2]) : "";
    xgui_draw_text_bold_at(g_xgui, (int)AS_INT(args[0]), (int)AS_INT(args[1]),
                           t, (int)AS_INT(args[3]), (uint32_t)AS_INT(args[4]));
    return value_null();
}
static Value bi_xgui_draw_text_bold_centered(Value *args, int argc) {
    if (!g_xgui || argc < 5) return value_null();
    const char *t = (VAL_TYPE(args[2]) == VAL_STRING) ? AS_STR(args[2]) : "";
    xgui_draw_text_bold_centered(g_xgui, (int)AS_INT(args[0]), (int)AS_INT(args[1]),
                                  t, (int)AS_INT(args[3]), (uint32_t)AS_INT(args[4]));
    return value_null();
}
/* ── key-hold queries ────────────────────────────────────────── */
...
static void register_builtins(Interpreter *interp) {
    ...
    struct { const char *name; BuiltinFn fn; } builtins[] = {
        ...
        {"xgui_card_begin",  bi_xgui_no_x11},
        {"xgui_card_end",    bi_xgui_no_x11},
        {"xgui_tooltip",     bi_xgui_no_x11},
#endif
        {NULL, NULL}
    };
...
static Value string_method(Interpreter *interp, Value obj, const char *method,
    ...
    if (strcmp(method, "find") == 0) {
        if (argc < 1 || VAL_TYPE(args[0]) != VAL_STRING) return value_int(-1);
    ...
    if (strcmp(method, "replace") == 0) {
        ...
        const char *old = AS_STR(args[0]), *neww = AS_STR(args[1]);
        size_t oldlen = strlen(old), newlen = strlen(neww);
        int cap = 256, sz = 0;
        char *res = malloc(cap);
        res[0] = '\0';
    ...
    if (strcmp(method, "join") == 0) {
        if (argc < 1 || VAL_TYPE(args[0]) != VAL_ARRAY) return value_retain(obj);
        ValueArray *items = &AS_ARRAY(args[0]);
        int cap = 256, sz = 0;
        char *res = malloc(cap);
        res[0] = '\0';
    ...
    if (strcmp(method, "endswith") == 0) {
        if (argc < 1 || VAL_TYPE(args[0]) != VAL_STRING) return value_bool(0);
        size_t sl = strlen(s), ml = strlen(AS_STR(args[0]));
        return value_bool((sl >= ml && strcmp(s + sl - ml, AS_STR(args[0])) == 0) ? 1 : 0);
    }
    ...
    if (strcmp(method, "count") == 0) {
        ...
        const char *needle = AS_STR(args[0]);
    ...
    if (strcmp(method, "pad_left") == 0 || strcmp(method, "ljust") == 0) {
        if (argc < 1 || VAL_TYPE(args[0]) != VAL_INT) return value_string(s);
        long long width = AS_INT(args[0]);
        char fill = (argc >= 2 && VAL_TYPE(args[1]) == VAL_STRING && AS_STR(args[1])[0]) ? AS_STR(args[1])[0] : ' ';
...
static Value array_method(Interpreter *interp, Value obj, const char *method,
    ...
    if (strcmp(method, "reverse") == 0) {
        ...
        for (int i = 0; i < n / 2; i++) {
            Value tmp = AS_ARRAY(obj).items[i];
            AS_ARRAY(obj).items[i] = AS_ARRAY(obj).items[n - 1 - i];
            AS_ARRAY(obj).items[n - 1 - i] = tmp;
        }
        return value_null();
    }
    ...
    if (strcmp(method, "join") == 0) {
        ...
        buf[0] = '\0';
        for (int i = 0; i < AS_ARRAY(obj).len; i++) {
            char *piece = value_to_string(AS_ARRAY(obj).items[i]);
...
static Value dict_method(Interpreter *interp, Value obj, const char *method,
    ...
    if (strcmp(method, "items") == 0) {
        ...
        for (int i = 0; i < AS_DICT(obj).len; i++) {
            Value pair_items[2] = { AS_DICT(obj).entries[i].key, AS_DICT(obj).entries[i].val };
    ...
    if (strcmp(method, "erase") == 0) {
        for (int i = 0; i < AS_DICT(obj).len; i++) {
            value_release(AS_DICT(obj).entries[i].key);
            value_release(AS_DICT(obj).entries[i].val);
        }
        AS_DICT(obj).len = 0;
        return value_null();
    }
    ...
    if (strcmp(method, "remove") == 0 || strcmp(method, "delete") == 0) {
        ...
        for (int i = 0; i < AS_DICT(obj).len; i++) {
            if (value_equals(AS_DICT(obj).entries[i].key, args[0])) {
                value_release(AS_DICT(obj).entries[i].key);
                value_release(AS_DICT(obj).entries[i].val);
                /* shift remaining entries down */
                for (int j = i; j < AS_DICT(obj).len - 1; j++)
                    AS_DICT(obj).entries[j] = AS_DICT(obj).entries[j + 1];
                AS_DICT(obj).len--;
                break;
            }
        }
...
static Value set_method(Interpreter *interp, Value obj, const char *method,
    ...
    if (strcmp(method, "update") == 0) {
        for (int a = 0; a < argc; a++) {
            if (VAL_TYPE(args[a]) == VAL_SET) {
                for (int i = 0; i < AS_SET(args[a]).len; i++) value_set_add(obj, AS_SET(args[a]).items[i]);
            }
        }
        return value_null();
    }
    ...
    if (strcmp(method, "pop") == 0) {
        ...
        Value item = AS_SET(obj).items[0];
        /* remove first */
        memmove(&AS_SET(obj).items[0], &AS_SET(obj).items[1], (AS_SET(obj).len-1)*sizeof(Value*));
        AS_SET(obj).len--;
        return item;
    }
    char emsg[128];
...
static Value do_slice(Interpreter *interp, Value obj, Value vstart, Value vstop, Value vstep, int line) {
    ...
    long long step  = vstep  ? (VAL_TYPE(vstep)  == VAL_INT ? AS_INT(vstep)  : 1) : 1;
    long long start = vstart ? (VAL_TYPE(vstart) == VAL_INT ? AS_INT(vstart) : (step > 0 ? 0 : len-1))
                             : (step > 0 ? 0 : len - 1);
    long long stop  = vstop  ? (VAL_TYPE(vstop)  == VAL_INT ? AS_INT(vstop)  : (step > 0 ? len : -len-1))
                             : (step > 0 ? len : -len - 1);
    ...
    Value result;
    ValueArray *src;
    bool is_tuple = (VAL_TYPE(obj) == VAL_TUPLE);
    if (is_tuple) { src = &AS_TUPLE(obj); result = VAL_SPEC_NULL; }
    else          { src = &AS_ARRAY(obj); result = value_array_new(); }
    ...
    for (long long i = start; step > 0 ? i < stop : i > stop; i += step) {
        ...
        if (is_tuple) {
            ...
            value_array_push(result, AS_ARRAY(src).items[i]);
        }
    }
    if (is_tuple) {
...
static Value eval_node(Interpreter *interp, ASTNode *node, Env *env) {
    ...
    case NODE_DICT_LIT: {
        ...
        for (int i = 0; i < node->dict_lit.count; i++) {
            ...
            if (VAL_TYPE(k) == VAL_STRING && !AS_PTR(k)->gc_immortal) {
                Value ik = gc_intern_string(gc_global(), AS_STR(k));
                value_release(k);
                k = ik;
            }
    ...
    case NODE_ASSIGN: {
        ...
        if ((target)->type == NODE_IDENT) {
            ...
            if (!interp->had_error && VAL_TYPE(obj) == VAL_DICT) {
    ...
    case NODE_FOR_IN: {
        ...
        if (VAL_TYPE(iter) == VAL_ARRAY) {
            for (int i = 0; i < AS_ARRAY(iter).len; i++) FOR_BODY(AS_ARRAY(iter).items[i]);
        } else if (VAL_TYPE(iter) == VAL_TUPLE) {
            for (int i = 0; i < AS_TUPLE(iter).len; i++) FOR_BODY(AS_TUPLE(iter).items[i]);
        } else if (VAL_TYPE(iter) == VAL_SET) {
            for (int i = 0; i < AS_SET(iter).len; i++) FOR_BODY(AS_SET(iter).items[i]);
        } else if (VAL_TYPE(iter) == VAL_STRING) {
            const char *s = AS_STR(iter);
            for (int i = 0; s[i]; i++) {
                char ch[2] = {s[i], '\0'};
                Value sv = value_string(ch);
                FOR_BODY(sv);
        ...
        } else if (VAL_TYPE(iter) == VAL_DICT) {
            for (int i = 0; i < AS_DICT(iter).len; i++) FOR_BODY(AS_DICT(iter).entries[i].key);
        } else {
    ...
    case NODE_FUNC_CALL: {
        ...
        for (int i = 0; i < raw_argc; i++) {
            ...
            if (arg_node->type == NODE_SPREAD) {
                ...
                if (VAL_TYPE(spread_val) == VAL_ARRAY) {
                    int need = actual_argc + AS_ARRAY(spread_val).len;
                    if (need > max_args) {
                        max_args = need + 8;
                        args = realloc(args, (size_t)max_args * sizeof(Value));
                    }
                    for (int j = 0; j < AS_ARRAY(spread_val).len; j++)
                        args[actual_argc++] = value_retain(AS_ARRAY(spread_val).items[j]);
        ...
        if (VAL_TYPE(callee) == VAL_BUILTIN) {
            ...
            result = AS_BUILTIN(callee).fn(args, actual_argc);
            g_current_interp = NULL;
        } else if (VAL_TYPE(callee) == VAL_FUNCTION) {
            Env *fn_env = env_new(AS_FUNC(callee).closure);
            for (int i = 0; i < AS_FUNC(callee).param_count; i++) {
                const char *pname = AS_FUNC(callee).params[i].name;
                ...
                if (i < actual_argc) {
                    arg = args[i];
                } else if (AS_FUNC(callee).params[i].default_val) {
                    /* evaluate default value expression in the function's closure */
                    arg = eval_node(interp, AS_FUNC(callee).params[i].default_val, AS_FUNC(callee).closure);
                    if (interp->had_error) { env_free(fn_env); goto call_cleanup; }
                } else {
                    arg = value_null();
                }
                env_set(fn_env, pname, arg, false);
                if (i >= actual_argc && !AS_FUNC(callee).params[i].default_val)
                    value_release(arg);
                else if (i >= actual_argc && AS_FUNC(callee).params[i].default_val)
                    value_release(arg);
            }
            ...
            value_release(eval_node(interp, AS_FUNC(callee).body, fn_env));
    ...
    case NODE_METHOD_CALL: {
        ...
        if (is_memory_module(obj)) {
            ...
            Value method_fn = VAL_SPEC_NULL;
            if (iof && VAL_TYPE(iof) == VAL_DICT) {
                ...
                if (!method_fn) {
                    ...
                    if (super_name && VAL_TYPE(super_name) == VAL_STRING) {
                        Value super_cls = env_get(env, AS_STR(super_name));
                        if (super_cls && VAL_TYPE(super_cls) == VAL_DICT) {
            ...
            if (method_fn && VAL_TYPE(method_fn) == VAL_FUNCTION) {
                /* Call the method with self=obj injected */
                Env *call_env = env_new(AS_FUNC(method_fn).closure);
                env_set(call_env, "self", obj, false);
                /* bind parameters (skip self if first param is named self) */
                int arg_offset = 0;
                if (AS_FUNC(method_fn).param_count > 0 &&
                    strcmp(AS_FUNC(method_fn).params[0].name, "self") == 0) {
                    arg_offset = 1;
                }
                for (int pi = arg_offset; pi < AS_FUNC(method_fn).param_count; pi++) {
                    const char *pname = AS_FUNC(method_fn).params[pi].name;
                    ...
                    } else if (AS_FUNC(method_fn).params[pi].default_val) {
                        argval = eval_node(interp, AS_FUNC(method_fn).params[pi].default_val, env);
                    } else {
                        argval = value_null();
                    }
                ...
                eval_node(interp, AS_FUNC(method_fn).body, call_env);
                result = value_null();
                if (interp->returning) {
                    interp->returning = false;
                ...
                if (ns_fn && VAL_TYPE(ns_fn) == VAL_BUILTIN) {
                    g_current_interp = interp;
                    result = AS_BUILTIN(ns_fn).fn(args, node->method_call.arg_count);
                    g_current_interp = NULL;
                    if (!result) result = value_null();
                } else if (ns_fn && VAL_TYPE(ns_fn) == VAL_FUNCTION) {
                    Env *ns_env = env_new(AS_FUNC(ns_fn).closure);
                    for (int pi = 0; pi < AS_FUNC(ns_fn).param_count; pi++) {
                        const char *pname = AS_FUNC(ns_fn).params[pi].name;
                        if (pname[0] == '.' && pname[1] == '.' && pname[2] == '.') {
                            const char *real_name = pname + 3;
                        ...
                        } else if (AS_FUNC(ns_fn).params[pi].default_val) {
                            av = eval_node(interp, AS_FUNC(ns_fn).params[pi].default_val,
                                           AS_FUNC(ns_fn).closure);
                    ...
                    interp->returning = false;
                    value_release(eval_node(interp, AS_FUNC(ns_fn).body, ns_env));
                    result = value_null();
                    if (interp->returning) {
                        value_release(result);
                        result = interp->return_val ? interp->return_val : value_null();
                    ...
                    if (result == VAL_SPEC_NULL) result = value_null();
                }
            }
        } else {
            switch (VAL_TYPE(obj)) {
                ...
                default: {
                    char emsg[64]; snprintf(emsg, sizeof(emsg), "type '%s' has no methods", value_type_name(VAL_TYPE(obj)));
                    runtime_error(interp, emsg, node->line);
                    result = value_null();
                }
            }
    ...
    case NODE_INDEX: {
        ...
        Value result = value_null();
        if (VAL_TYPE(obj) == VAL_ARRAY) {
            if (VAL_TYPE(idx) != VAL_INT) { runtime_error(interp, "array index must be an integer", node->line); }
            else {
                Value item = value_array_get(obj, AS_INT(idx));
            ...
            if (VAL_TYPE(idx) != VAL_INT) { runtime_error(interp, "tuple index must be an integer", node->line); }
            else {
                long long i = AS_INT(idx);
            ...
            if (VAL_TYPE(idx) != VAL_INT) { runtime_error(interp, "string index must be an integer", node->line); }
            else {
                long long i = AS_INT(idx);
    ...
    case NODE_SLICE: {
        ...
        Value start = node->slice_expr.start ? eval_node(interp, node->slice_expr.start, env) : VAL_SPEC_NULL;
        Value stop  = node->slice_expr.stop  ? eval_node(interp, node->slice_expr.stop,  env) : VAL_SPEC_NULL;
        Value step  = node->slice_expr.step  ? eval_node(interp, node->slice_expr.step,  env) : VAL_SPEC_NULL;
        if (interp->had_error) {
    ...
    case NODE_IN_EXPR: {
        ...
        bool found = false;
        if (VAL_TYPE(cont) == VAL_STRING && VAL_TYPE(item) == VAL_STRING) {
            found = strstr(AS_STR(cont), AS_STR(item)) != NULL;
        } else if (VAL_TYPE(cont) == VAL_ARRAY) {
            for (int i = 0; i < AS_ARRAY(cont).len; i++) { if (value_equals(AS_ARRAY(cont).items[i], item)) { found = true; break; } }
        } else if (VAL_TYPE(cont) == VAL_SET) {
            found = value_set_has(cont, item);
        } else if (VAL_TYPE(cont) == VAL_TUPLE) {
            for (int i = 0; i < AS_TUPLE(cont).len; i++) { if (value_equals(AS_TUPLE(cont).items[i], item)) { found = true; break; } }
        } else if (VAL_TYPE(cont) == VAL_DICT) {
            for (int i = 0; i < AS_DICT(cont).len; i++) { if (value_equals(AS_DICT(cont).entries[i].key, item)) { found = true; break; } }
        }
    ...
    case NODE_UNOP: {
        ...
        if (strcmp(op, "-") == 0) {
            ...
            if (VAL_TYPE(operand) == VAL_INT) { value_release(result); result = value_int(~AS_INT(operand)); }
    ...
    case NODE_BINOP: {
        ...
        if (strcmp(op, "+")  == 0) { Value r = value_add(left, right); if (r) { value_release(result); result = r; } else runtime_error(interp, "unsupported operands for '+'", node->line); }
            ...
            if (VAL_TYPE(left) == VAL_INT && VAL_TYPE(right) == VAL_INT) {
                if (AS_INT(right) == 0) { runtime_error(interp, "floor division by zero", node->line); }
                else { value_release(result); result = value_int(AS_INT(left) / AS_INT(right)); }
            } else if ((VAL_TYPE(left) == VAL_FLOAT || VAL_TYPE(left) == VAL_INT) &&
                       (VAL_TYPE(right) == VAL_FLOAT || VAL_TYPE(right) == VAL_INT)) {
                double lv = VAL_TYPE(left)  == VAL_FLOAT ? AS_FLOAT(left)  : (double)AS_INT(left);
                double rv = VAL_TYPE(right) == VAL_FLOAT ? AS_FLOAT(right) : (double)AS_INT(right);
            ...
            if (VAL_TYPE(left) == VAL_INT && VAL_TYPE(right) == VAL_INT) { value_release(result); result = value_int(AS_INT(left) & AS_INT(right)); }
            else if (VAL_TYPE(left) == VAL_SET && VAL_TYPE(right) == VAL_SET) {
                value_release(result); result = value_set_new();
                for (int i = 0; i < AS_SET(left).len; i++)
                    if (value_set_has(right, AS_SET(left).items[i])) value_set_add(result, AS_SET(left).items[i]);
            }
        }
        else if (strcmp(op, "|")  == 0) {
            if (VAL_TYPE(left) == VAL_INT && VAL_TYPE(right) == VAL_INT) { value_release(result); result = value_int(AS_INT(left) | AS_INT(right)); }
                ...
                for (int i = 0; i < AS_SET(right).len; i++) value_set_add(result, AS_SET(right).items[i]);
            }
        }
        else if (strcmp(op, "^")  == 0) {
            if (VAL_TYPE(left) == VAL_INT && VAL_TYPE(right) == VAL_INT) { value_release(result); result = value_int(AS_INT(left) ^ AS_INT(right)); }
                ...
                for (int i = 0; i < AS_SET(left).len; i++)
                    if (!value_set_has(right, AS_SET(left).items[i])) value_set_add(result, AS_SET(left).items[i]);
                for (int i = 0; i < AS_SET(right).len; i++)
                    if (!value_set_has(left, AS_SET(right).items[i])) value_set_add(result, AS_SET(right).items[i]);
            }
        }
        else if (strcmp(op, "<<") == 0) {
            if (VAL_TYPE(left) == VAL_INT && VAL_TYPE(right) == VAL_INT) {
                long long sh = AS_INT(right);
                value_release(result);
                result = (sh >= 0 && sh < 64) ? value_int(AS_INT(left) << sh) : value_int(0);
    ...
    case NODE_REPEAT: {
        ...
        if (node->repeat_stmt.count) {
            ...
            long long n = (VAL_TYPE(cnt_val) == VAL_INT) ? AS_INT(cnt_val) : 0;
    ...
    case NODE_MATCH: {
        ...
        for (int i = 0; i < node->match_stmt.count && !matched; i++) {
            ...
            bool match_ok = false;
            if (VAL_TYPE(pat_val) == VAL_ARRAY && (pat)->type == NODE_RANGE) {
                /* Range match: check if val is in range */
                if (AS_ARRAY(pat_val).len == 0) {
                    match_ok = false;
                } else {
                    /* ranges are stored as an array of values when iterated */
                    for (int j = 0; j < AS_ARRAY(pat_val).len; j++) {
                        if (value_equals(val, AS_ARRAY(pat_val).items[j])) { match_ok = true; break; }
                    }
                }
            } else {
    ...
    case NODE_RANGE: {
        ...
        if (node->range_lit.step) {
            ...
            if (VAL_TYPE(step_val) == VAL_INT) step_n = AS_INT(step_val);
        ...
        if (VAL_TYPE(start) == VAL_INT && VAL_TYPE(end) == VAL_INT) {
            long long a = AS_INT(start), b = AS_INT(end);
            if (step_n > 0) {
        ...
        } else if (VAL_TYPE(start) == VAL_FLOAT || VAL_TYPE(end) == VAL_FLOAT) {
            double a = (VAL_TYPE(start) == VAL_FLOAT ? AS_FLOAT(start) : (double)AS_INT(start));
            double b = (VAL_TYPE(end)   == VAL_FLOAT ? AS_FLOAT(end)   : (double)AS_INT(end));
        ...
        } else if (VAL_TYPE(start) == VAL_STRING && VAL_TYPE(end) == VAL_STRING) {
            /* character range: "a".."z" */
            char sc = AS_STR(start)[0], ec = AS_STR(end)[0];
    ...
    case NODE_NULLCOAL: {
        ...
        bool is_null_like = (VAL_TYPE(left) == VAL_NULL) ||
                            (VAL_TYPE(left) == VAL_BOOL && AS_BOOL(left) == -1);
        if (!is_null_like) {
            return left;
        }
        value_release(left);
    ...
    case NODE_IS_EXPR: {
        ...
        bool match_ok = false;
        if (strcmp(tname, "int")     == 0) match_ok = (VAL_TYPE(obj) == VAL_INT);
        else if (strcmp(tname, "float")   == 0) match_ok = (VAL_TYPE(obj) == VAL_FLOAT);
        else if (strcmp(tname, "str")     == 0) match_ok = (VAL_TYPE(obj) == VAL_STRING);
        else if (strcmp(tname, "bool")    == 0) match_ok = (VAL_TYPE(obj) == VAL_BOOL);
        else if (strcmp(tname, "null")    == 0) match_ok = (VAL_TYPE(obj) == VAL_NULL);
        else if (strcmp(tname, "arr")     == 0) match_ok = (VAL_TYPE(obj) == VAL_ARRAY);
        else if (strcmp(tname, "array")   == 0) match_ok = (VAL_TYPE(obj) == VAL_ARRAY);
        else if (strcmp(tname, "dict")    == 0) match_ok = (VAL_TYPE(obj) == VAL_DICT);
        else if (strcmp(tname, "set")     == 0) match_ok = (VAL_TYPE(obj) == VAL_SET);
        else if (strcmp(tname, "tuple")   == 0) match_ok = (VAL_TYPE(obj) == VAL_TUPLE);
        else if (strcmp(tname, "unknown") == 0) match_ok = (VAL_TYPE(obj) == VAL_BOOL && AS_BOOL(obj) == -1);
        else {
            /* check type() string */
            char *tn = value_to_string(obj);
            /* compare by value_type_name */
            const char *actual = value_type_name(VAL_TYPE(obj));
    ...
    case NODE_NEW_EXPR: {
        ...
        if (is_struct && value_truthy(is_struct)) {
            ...
            if (fields_arr2 && VAL_TYPE(fields_arr2) == VAL_ARRAY) {
                for (int i = 0; i < AS_ARRAY(fields_arr2).len && i < node->new_expr.arg_count; i++) {
                    Value fkey = AS_ARRAY(fields_arr2).items[i];
                ...
                for (int i = node->new_expr.arg_count; i < AS_ARRAY(fields_arr2).len; i++) {
                    Value fkey = AS_ARRAY(fields_arr2).items[i];
            ...
            if (init_fn && VAL_TYPE(init_fn) == VAL_FUNCTION) {
                /* Call init with self=instance */
                Env *call_env = env_new(AS_FUNC(init_fn).closure);
                env_set(call_env, "self", instance, false);
                /* bind parameters to args */
                for (int i = 0; i < AS_FUNC(init_fn).param_count; i++) {
                    const char *pname = AS_FUNC(init_fn).params[i].name;
                    ...
                    if (i < AS_FUNC(init_fn).param_count &&
                        strcmp(AS_FUNC(init_fn).params[0].name, "self") == 0) {
                        arg_offset = 1;
                    }
                    int ai = i - arg_offset;
                    ...
                    } else if (AS_FUNC(init_fn).params[i].default_val) {
                        value_release(argval);
                        argval = eval_node(interp, AS_FUNC(init_fn).params[i].default_val, env);
                    }
                    env_set(call_env, pname, argval, false);
                    value_release(argval);
                }
                eval_node(interp, AS_FUNC(init_fn).body, call_env);
                interp->returning = false;
                if (interp->return_val) { value_release(interp->return_val); interp->return_val = VAL_SPEC_NULL; }
                env_free(call_env);
